...
$(document).ready(function () {
	...
	table = $('#datatable').DataTable({
		...
		"ajax": {
			"url": '<?php echo base_url('jsonkategori'); ?>',
			"type": "POST"
		},
...
function tambah() {
	...
	$("#modalbody").load("modalkategori/", function (data) {
		$("#modalbody").html(data);
	});
}
...
function ganti(id) {
	...
	$("#modalbody").load("<?php echo base_url();?>editkategori/" + id, function (data) {
		$("#modalbody").html(data);
	});
}
$("#form").on('submit', (function (e) {
	...
	var url;
	if (simpan == 'tambah') {
		url = "<?php echo base_url();?>addkategori";
	} else {
		url = "<?php echo base_url();?>updatekategori";
	}
	$.ajax({
		url: url,
		type: "POST",
...
function hapus(id) {
	swal({
		...
		$.ajax({
			url: "<?php echo site_url('deletekategori')?>/" + id,
			type: "POST",
			dataType: "JSON",
			success: function (data) {
				table.ajax.reload();
