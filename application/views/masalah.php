                     ...
                     <div class="tab-content">
                        ...
                        <div class="tab-pane active" id="profile1">
                           <form id="carinilai">
                              ...
                              <div class="col-md-2">
                                 <input readonly="true" value="<?php echo $dari;?>" id="dari" name="dari" class="form-control" type="text" required>
                              </div>
                              <div class="col-md-2">
                                 <input readonly="true" value="<?php echo $sampai;?>" class="form-control" name="sampai" id="sampai" type="text">
                              </div>
...
<!-- Init js -->
<script>
var currentTime=new Date();$(function(){$('#dari').datepicker({changeMonth:!0,changeYear:!0,showButtonPanel:!0,dateFormat:'yy-mm',minDate:new Date('2018-01'),onClose:function(dateText,inst){$(this).datepicker('setDate',new Date(inst.selectedYear,inst.selectedMonth,1))}});var x=document.getElementById("dari").value;x=new Date(x);x.setDate(x.getDate());$('#sampai').datepicker({changeMonth:!0,changeYear:!0,showButtonPanel:!0,dateFormat:'yy-mm',minDate:x,onClose:function(dateText,inst){$(this).datepicker('setDate',new Date(inst.selectedYear,inst.selectedMonth,1))}});$('#dari1').datepicker({changeMonth:!0,changeYear:!0,showButtonPanel:!0,dateFormat:'yy-mm',minDate:new Date('2018-01'),onClose:function(dateText,inst){$(this).datepicker('setDate',new Date(inst.selectedYear,inst.selectedMonth,1))}}).datepicker("setDate",currentTime);var x=document.getElementById("dari").value;x=new Date(x);x.setDate(x.getDate());$('#sampai1').datepicker({changeMonth:!0,changeYear:!0,showButtonPanel:!0,dateFormat:'yy-mm',minDate:x,onClose:function(dateText,inst){$(this).datepicker('setDate',new Date(inst.selectedYear,inst.selectedMonth,1))}}).datepicker("setDate",currentTime)})
</script>
<script>
...
<?php if($bulan != NULL) { ?>
<script>
$.ajax({url:"<?php echo base_url();?>carinilai",type:"POST",data:$('#carinilai').serialize(),success:function(data)
{$("#hasilcari").html(data)},error:function(jqXHR,textStatus,errorThrown)
{swal("Error!","","error")}})
</script>
<?php } ?>