...
<script type="text/javascript">
   ...
   var simpan;
   		...
   		//keys: true,
                    ...
                    "ajax": {
                        "url": '<?php echo base_url('jsonuser'); ?>',
                        "type": "POST"
                    },
   ...
   $("#modalbody").load("modaluser/",function(data){
        $("#modalbody").html(data);
     });
   ...
   function ganti(id) {
    ...
    $('#form')[0].reset(); // reset form on modals
   $('#myModal').modal('show'); // show bootstrap modal
   $("#modalbody").load("<?php echo base_url();?>edituser/"+id,function(data){
        $("#modalbody").html(data);
     });
   /*$("#form").on('submit',(function(e) {
   e.preventDefault();
   ...
   function hapus(id) {
    swal({
   	...
   	$.ajax({
   		url : "<?php echo site_url('deleteuser')?>/"+id,
   		type: "POST",
   		dataType: "JSON",
   		success: function(data)
   		{
